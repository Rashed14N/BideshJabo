  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-extrabold tracking-tight dark:text-white">Announcements</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Manage global notifications and updates for all students.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 gap-4">
            ...
            <div className="p-6 flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div className="flex gap-4">
                <div className={cn(
                  ...
                  ann.type === 'info' ? "bg-blue-50 dark:bg-blue-900/20 text-blue-primary dark:text-gold" : 
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">{ann.message}</p>
                  <div className="flex items-center gap-4 pt-2">
                    <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 font-medium">
                      <Clock size={14} /> {ann.createdAt?.toDate().toLocaleString() || "Just now"}
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 font-medium uppercase tracking-widest">
                      Type: {ann.type}
                    </div>
                  </div>
                </div>
        ...
        {announcements.length === 0 && !loading && (
          <div className="bg-white dark:bg-[#1A1A1A] border border-dashed border-slate-300 dark:border-slate-700 p-12 rounded-2xl text-center">
            ...
            <h3 className="text-lg font-bold text-slate-400 dark:text-slate-500">No announcements yet</h3>
            <p className="text-slate-400 dark:text-slate-500 text-sm">Send your first global update to all students.</p>
          </div>
        )}
      </div>
